...
def get_longitudinal_data_nosub(samples, lrandom=False, add_feats=0, n_time_steps=20):
    Xvals = np.arange(n_time_steps)
    if not lrandom: 
...
def get_longitudinal_data(stypelist, lrandom=False, add_feats=0, n_time_steps=20):
    Xvals = np.arange(n_time_steps)
    if not lrandom: 
        up = [-2, 0.0002, 0.1]; down = [4, -0.1, -0.1]
    else: 
...
def generate_synthetic(nsamples, noisy_ys= False, lrandom=False, add_feats=0, sub=True, n_time_steps=20):
    # Generate Data
    ...
    if noisy_ys:
        ...
        ys              = get_ys(subtype)
    if sub: 
        ldata           = get_longitudinal_data(subtype, lrandom, add_feats, n_time_steps)
    else: 
        ldata           = get_longitudinal_data_nosub(subtype, lrandom, add_feats, n_time_steps)
...
""" Function to return data to feed into models """
def load_synthetic_data_trt(
    fold_span = range(5), 
    nsamples = {'train':500, 'valid':300, 'test':200},
    distractor_dims_b = 0, 
    sigma_ys = 0.1, 
    include_line = True, 
    lrandom = False,
